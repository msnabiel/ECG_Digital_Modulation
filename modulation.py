...
num_bits_display = 3  # Number of bits to display in each frame
t = np.arange(0, num_bits_display * bit_duration, 1/fs)  # Time vector for 3 bits window
...
# Plotting limits to show three bits
for ax in axs:
    ax.set_xlim(0, num_bits_display * bit_duration)
    ax.set_ylim(-1.5, 1.5)
    ax.grid()
...
def update(frame):
    ...
    if len(data_bits) < (frame + num_bits_display):
        data_bits.extend([0] * (frame + num_bits_display - len(data_bits)))  # Pad with 0s if not enough data

    # Select the last three bits for the current frame
    current_bits = data_bits[frame:frame + num_bits_display]
    digital_signal_segment = np.repeat(current_bits, len(t) // len(current_bits))
